...
ST_FUNC void relocate_plt( TCCState *s1 )
{
    ...
    if( ind < end_offset ) {
        ...
        uint64_t off = IMM_HIGH( got - plt );

        if( ( off + ( (uint32_t)1 << 20 ) ) >> 21 )
            tcc_error_noabort( "Failed relocating PLT (off=0x%lx, got=0x%lx, plt=0x%lx)", (long)off,
                (long)got, (long)plt );
        ...
        while( ind < end_offset ) {
            ...
            if( ( off + ( (uint32_t)1 << 20 ) ) >> 21 )
                tcc_error_noabort( "Failed relocating PLT (off=0x%lx, addr=0x%lx, pc=0x%lx)", (long)off,
                    (long)addr, (long)pc );
