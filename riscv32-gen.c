    ...
    // Value is stored in the CPU flag from a comparison operation. Put it in a safe place.
        ...
        switch (op) {
            ...
            case TOK_GT:
                ...
                if (inv) { emit_XORI(rd, rd, 1); } // xori d, d, 1
                break;
            case TOK_NE:
            case TOK_EQ:
                if (rd != a || b) {
                    emit_SUB(rd, a, b); // sub d, a, b
                }
                if (op == TOK_NE) {
                    emit_SLTU(r, 0, rd); // sltu d, x0, d == snez d,d
                } else {
                    emit_SLTIU(rd, rd, 1); // sltiu d, d, 1 == seqz d,d
                }
                break;
        }
    }
        ...
        gjmp_addr(ind + 8);
        gsym(lvar_offset);
        emit_ADDI(rd, 0, t^1); // addi Rd, x0, !t
    }
    else {
